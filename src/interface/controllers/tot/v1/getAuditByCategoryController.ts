...
import {
  ...
  Team,
} from "../../../../domain/entities/tot/v1/getAuditByCategory.js"
    ...
    const getTaskCategoryQuery = `
        ...
        WHERE
          TA.TASK_ID = T.TASK_ID
              ...
              ${startDateFilter}
              ${endDateFilter}
        GROUP BY TA2.TASK_AUDIT_ID , TT.TASK_CATEGORY_NAME, TT.TASK_CATEGORY_ID, T.TASK_STATUS_ID; `
...
const getTaskCategory = async (
  ...
  try {
    ...
    tasksCategorysReport.map((objEachDataSet) => {
      ...
      const taskCategoryId = objEachDataSet["task-category-id"]

      if (typeof arrDifferentCategories[taskCategoryId] === "undefined") {
        arrDifferentCategories[taskCategoryId] = {
          "task-category-id": taskCategoryId,
          "task-category-name": taskCategoryName,
          audits: [],
        }
      }
      if (typeof arrDifferentTasksInCategories[taskCategoryId] === "undefined") {
        arrDifferentTasksInCategories[taskCategoryId] = []
      }
      arrDifferentTasksInCategories[taskCategoryId].push({
        ...objEachDataSet,
      })
      return {}
    })
    ...
    arrDifferentTasksInCategories.map((arrEachTaskInCategory, categoryID) => {
      if (typeof mappedTaskAudits[categoryID] === "undefined") {
        mappedTaskAudits[categoryID] = []
      }
      arrEachTaskInCategory.map((objIndividualTask: any) => {
        ...
        if (typeof mappedTaskAudits[categoryID][thisTaskId] === "undefined") {
          mappedTaskAudits[categoryID][thisTaskId] = []
        }
        ...
        mappedTaskAudits[categoryID][thisTaskId].push({ ...objToPush })
      })
    })

    /** removing null elements from array */
    mappedTaskAudits = mappedTaskAudits.map((arrttt) => {
      return arrttt.filter((n: any) => n)
    })

    /** Creating seperate final response to the output */
    let getTaskCategoryResponse: any[] = []
    getTaskCategoryResponse = arrDifferentCategories.map((objCat, categoryID) => {
      const tmpObj = {
        "task-category-id": objCat["task-category-id"],
        "task-category-name": objCat["task-category-name"],
        audits: mappedTaskAudits[categoryID].map((objEachTaskGroup: any) => {
          return {
...
export const consolidategetTaskCategoryReportRequest = (
  req: Request,
): getTaskCategoryRequest | poorlyShapedRequest => ({
...
export const taskCategoryControllerContolller = jsonResponse(
  extractValue(consolidategetTaskCategoryReportRequest)(getTaskCategory),
)
