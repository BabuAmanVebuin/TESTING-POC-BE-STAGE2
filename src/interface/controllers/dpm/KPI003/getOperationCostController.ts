...
import { KPI003APIRequestParams, OperationCostJson } from "../../../../domain/models/dpm/KPI003/Index.js"
...
import { generateOperationCostResponseUseCase } from "../../../../application/use_cases/dpm/generateOperationCostResponseUseCase.js"
...
export const getOperationCostController = async <X>(
  ...
  t: any,
): Promise<Either<ApplicationError, OperationCostJson>> => {
  /**
   * Validation
   */
  ...
  let cachedJson = await wrapInTransaction((transaction) =>
    getCacheUseCase<X, OperationCostJson>(
  ...
  if (cachedJson === null) {
    const newCachedJson = await generateOperationCostResponseUseCase(
      validationResult.right.plantCode,
      validationResult.right.unitCode,
      pointInTime.toSeconds(),
      kpi003Repository,
    ...
    await wrapInTransaction((transaction) =>
      saveCacheUseCase<X, OperationCostJson>(
        kpiResponseCacheRepository,
        validationResult.right.plantCode,
        validationResult.right.unitCode,
        pointInTime.toSeconds(),
    ...
    cachedJson.OperationCost.Annual.Suffix =
      cachedJson.OperationCost.Monthly.Suffix =
      cachedJson.OperationCost.Weekly.Suffix =
      cachedJson.OperationCost.Cumulative.Suffix =
        t("VALUE.SUFFIX_OKU")
    cachedJson.OperationCost.Annual.Prefix =
      cachedJson.OperationCost.Monthly.Prefix =
      cachedJson.OperationCost.Weekly.Prefix =
      cachedJson.OperationCost.Daily.Prefix =
      cachedJson.OperationCost.Cumulative.Prefix =
        t("VALUE.PREFIX_YEN")

    cachedJson.OperationCost.Daily.Suffix = t("VALUE.SUFFIX_MAN")
  }

  // Generate the Kpi003 operationcost JSON
