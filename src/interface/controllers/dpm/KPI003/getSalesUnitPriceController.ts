...
import { KPI003APIRequestParams, SalesUnitPriceJson } from "../../../../domain/models/dpm/KPI003/Index.js"
...
import { generateSalesUnitPriceResponseUseCase } from "../../../../application/use_cases/dpm/generateSalesUnitPriceResponseUseCase.js"
...
export const getSalesUnitPriceController = async <X>(
  ...
  t: any,
): Promise<Either<ApplicationError, SalesUnitPriceJson>> => {
  /**
   * Validation
   */
  ...
  let cachedJson = await wrapInTransaction((transaction) =>
    getCacheUseCase<X, SalesUnitPriceJson>(
  ...
  if (cachedJson === null) {
    const newCachedJson = await generateSalesUnitPriceResponseUseCase(
      validationResult.right.plantCode,
      validationResult.right.unitCode,
      pointInTime.toSeconds(),
      kpi003Repository,
    ...
    await wrapInTransaction((transaction) =>
      saveCacheUseCase<X, SalesUnitPriceJson>(
        kpiResponseCacheRepository,
        validationResult.right.plantCode,
        validationResult.right.unitCode,
        pointInTime.toSeconds(),
    ...
    cachedJson.SalesUnitPrice.Annual.Suffix =
      cachedJson.SalesUnitPrice.Monthly.Suffix =
      cachedJson.SalesUnitPrice.Weekly.Suffix =
      cachedJson.SalesUnitPrice.Daily.Suffix =
      cachedJson.SalesUnitPrice.Cumulative.Suffix =
        t("VALUE.SUFFIX_YEN_KWH")
  }
  // Generate the Kpi003 SalesUnitPrice JSON
