...
import { KPI003APIRequestParams, GenerationOutputPPAJson } from "../../../../domain/models/dpm/KPI003/Index.js"
...
import { generateGenerationOutputPPAResponseUseCase } from "../../../../application/use_cases/dpm/generateGenerationOutputPPAResponseUseCase.js"
...
export const getGenerationOutputPPAController = async <X>(
  ...
  t: any,
): Promise<Either<ApplicationError, GenerationOutputPPAJson>> => {
  /**
   * Validation
   */
  ...
  let cachedJson = await wrapInTransaction((transaction) =>
    getCacheUseCase<X, GenerationOutputPPAJson>(
  ...
  if (cachedJson === null) {
    const newCachedJson = await generateGenerationOutputPPAResponseUseCase(
      validationResult.right.plantCode,
      validationResult.right.unitCode,
      pointInTime.toSeconds(),
      kpi003Repository,
    ...
    await wrapInTransaction((transaction) =>
      saveCacheUseCase<X, GenerationOutputPPAJson>(
        kpiResponseCacheRepository,
        validationResult.right.plantCode,
        validationResult.right.unitCode,
        pointInTime.toSeconds(),
    ...
    cachedJson.GenerationOutputPPA.Annual.Suffix =
      cachedJson.GenerationOutputPPA.Monthly.Suffix =
      cachedJson.GenerationOutputPPA.Weekly.Suffix =
      cachedJson.GenerationOutputPPA.Cumulative.Suffix =
        t("VALUE.GWH")

    cachedJson.GenerationOutputPPA.Daily.Suffix = t("VALUE.MWH")
  }
