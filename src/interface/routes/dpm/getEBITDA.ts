...
import { getEBITDAController } from "../../controllers/dpm/KPI003/getEBITDAController.js"

// Function to handle the result of the EBITDA API request
const handleResult = (res: Response, ebitdaJson: Either<ApplicationError, EBITDAJson>): void => {
  if (isRight(ebitdaJson)) {
    logResponse(ebitdaJson.right)
    res.status(HTTP_STATUS.OK).send(ebitdaJson.right)
  } else {
    const error: ApplicationError = ebitdaJson.left
    switch (error._tag) {
...
// Handler for the GET EBITDA API endpoint
export const getEBITDA = (router: Router): void => {
  router.get(
    "/",
    asyncWrapper(async (req, res) => {
      logger.info(`GET EBITDA API started`) // Logging a message indicating the start of the EBITDA API request
      ...
      await wrapInSnowflakeTransaction(async (snowflakeTransaction) => {
        ...
        const ebitdaJson = await getEBITDAController(
          {
