...
import { getGenerationOutputPPAController } from "../../controllers/dpm/KPI003/getGenerationOutputPPAController.js"

// Function to handle the result of the GenerationOutputPPA API request
const handleResult = (
  res: Response,
  generationOutputPPAJson: Either<ApplicationError, GenerationOutputPPAJson>,
): void => {
  if (isRight(generationOutputPPAJson)) {
    logResponse(generationOutputPPAJson.right)
    res.status(HTTP_STATUS.OK).send(generationOutputPPAJson.right)
  } else {
    const error: ApplicationError = generationOutputPPAJson.left
    switch (error._tag) {
...
// Handler for the GET GenerationOutputPPA API endpoint
export const getGenerationOutputPPA = (router: Router): void => {
  router.get(
    "/",
    asyncWrapper(async (req, res) => {
      logger.info(`GET GenerationOutputPPA API started`) // Logging a message indicating the start of the GenerationOutputPPA API request
      ...
      await wrapInSnowflakeTransaction(async (snowflakeTransaction) => {
        ...
        const generationOutputPPAJson = await getGenerationOutputPPAController(
          {
        ...
        handleResult(res, generationOutputPPAJson)
      })
    }),
  )
}
