...
import { getGrossMarginPPAController } from "../../controllers/dpm/KPI003/getGrossMarginPPAController.js"

// Function to handle the result of the GrossMargin API request
const handleResult = (res: Response, grossmarginPPAJson: Either<ApplicationError, GrossMarginPPAJson>): void => {
  if (isRight(grossmarginPPAJson)) {
    logResponse(grossmarginPPAJson.right)
    res.status(HTTP_STATUS.OK).send(grossmarginPPAJson.right)
  } else {
    const error: ApplicationError = grossmarginPPAJson.left
    switch (error._tag) {
...
export const getGrossMarginPPA = (router: Router): void => {
  router.get(
    "/",
    asyncWrapper(async (req, res) => {
      ...
      await wrapInSnowflakeTransaction(async (snowflakeTransaction) => {
        ...
        const grossmarginPPAJson = await getGrossMarginPPAController(
          {
        ...
        handleResult(res, grossmarginPPAJson)
      })
    }),
  )
}
