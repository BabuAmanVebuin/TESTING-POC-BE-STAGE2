...
import { getBasicProfitController } from "../../controllers/dpm/KPI003/getBasicProfitController.js"

// Function to handle the result of the BasicProfit API request
const handleResult = (res: Response, basicprofitJson: Either<ApplicationError, BasicProfitJson>): void => {
  if (isRight(basicprofitJson)) {
    logResponse(basicprofitJson.right)
    res.status(HTTP_STATUS.OK).send(basicprofitJson.right)
  } else {
    const error: ApplicationError = basicprofitJson.left
    switch (error._tag) {
...
// Handler for the GET BasicProfit API endpoint
export const getBasicProfit = (router: Router): void => {
  router.get(
    "/",
    asyncWrapper(async (req, res) => {
      logger.info(`GET BasicProfit API started`) // Logging a message indicating the start of the BasicProfit API request
      ...
      await wrapInSnowflakeTransaction(async (snowflakeTransaction) => {
        ...
        const basicprofitJson = await getBasicProfitController(
          {
        ...
        handleResult(res, basicprofitJson)
      })
    }),
  )
}
