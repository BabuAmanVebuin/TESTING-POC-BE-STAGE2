...
import { getMaintenanceCostController } from "../../controllers/dpm/KPI003/getMaintenanceCostController.js"

// Function to handle the result of the MaintenanceCost API request
const handleResult = (res: Response, maintenancecostJson: Either<ApplicationError, MaintenanceCostJson>): void => {
  if (isRight(maintenancecostJson)) {
    logResponse(maintenancecostJson.right)
    res.status(HTTP_STATUS.OK).send(maintenancecostJson.right)
  } else {
    const error: ApplicationError = maintenancecostJson.left
    switch (error._tag) {
...
// Handler for the GET MaintenanceCost API endpoint
export const getMaintenanceCost = (router: Router): void => {
  router.get(
    "/",
    asyncWrapper(async (req, res) => {
      logger.info(`GET MaintenanceCost API started`) // Logging a message indicating the start of the MaintenanceCost API request
      ...
      await wrapInSnowflakeTransaction(async (snowflakeTransaction) => {
        ...
        const maintenancecostJson = await getMaintenanceCostController(
          {
        ...
        handleResult(res, maintenancecostJson)
      })
    }),
  )
}
