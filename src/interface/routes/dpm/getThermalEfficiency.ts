...
import { getThermalEfficiencyController } from "../../controllers/dpm/KPI003/getThermalEfficiencyController.js"

// Function to handle the result of the ThermalEfficiency API request
const handleResult = (res: Response, thermalEfficiencyJson: Either<ApplicationError, ThermalEfficiencyJson>): void => {
  if (isRight(thermalEfficiencyJson)) {
    logResponse(thermalEfficiencyJson.right)
    res.status(HTTP_STATUS.OK).send(thermalEfficiencyJson.right)
  } else {
    const error: ApplicationError = thermalEfficiencyJson.left
    switch (error._tag) {
...
// Handler for the GET ThermalEfficiency API endpoint
export const getThermalEfficiency = (router: Router): void => {
  router.get(
    "/",
    asyncWrapper(async (req, res) => {
      logger.info(`GET ThermalEfficiency API started`) // Logging a message indicating the start of the ThermalEfficiency API request
      ...
      await wrapInSnowflakeTransaction(async (snowflakeTransaction) => {
        ...
        const thermalEfficiencyJson = await getThermalEfficiencyController(
          {
        ...
        handleResult(res, thermalEfficiencyJson)
      })
    }),
  )
}
