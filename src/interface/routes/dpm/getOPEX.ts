...
import { getOPEXController } from "../../controllers/dpm/KPI003/getOPEXController.js"

// Function to handle the result of the OPEX API request
const handleResult = (res: Response, opexJson: Either<ApplicationError, OPEXJson>): void => {
  if (isRight(opexJson)) {
    logResponse(opexJson.right)
    res.status(HTTP_STATUS.OK).send(opexJson.right)
  } else {
    const error: ApplicationError = opexJson.left
    switch (error._tag) {
...
// Handler for the GET OPEX  API endpoint
export const getOPEX = (router: Router): void => {
  router.get(
    "/",
    asyncWrapper(async (req, res) => {
      logger.info(`GET OPEX  API started`) // Logging a message indicating the start of the OPEX API request
      ...
      await wrapInSnowflakeTransaction(async (snowflakeTransaction) => {
        ...
        const opexJson = await getOPEXController(
          {
