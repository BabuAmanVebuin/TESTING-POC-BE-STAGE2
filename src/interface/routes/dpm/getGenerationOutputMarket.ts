...
import { getGenerationOutputMarketController } from "../../controllers/dpm/KPI003/getGenerationOutputMarketController.js"

// Function to handle the result of the GenerationOutputMarket API request
const handleResult = (
  res: Response,
  generationOutputMarketJson: Either<ApplicationError, GenerationOutputMarketJson>,
): void => {
  if (isRight(generationOutputMarketJson)) {
    logResponse(generationOutputMarketJson.right)
    res.status(HTTP_STATUS.OK).send(generationOutputMarketJson.right)
  } else {
    const error: ApplicationError = generationOutputMarketJson.left
    switch (error._tag) {
...
// Handler for the GET GenerationOutputMarket API endpoint
export const getGenerationOutputMarket = (router: Router): void => {
  router.get(
    "/",
    asyncWrapper(async (req, res) => {
      logger.info(`GET GenerationOutputMarket API started`) // Logging a message indicating the start of the GenerationOutputMarket API request
      ...
      await wrapInSnowflakeTransaction(async (snowflakeTransaction) => {
        ...
        const generationOutputMarketJson = await getGenerationOutputMarketController(
          {
        ...
        handleResult(res, generationOutputMarketJson)
      })
    }),
  )
}
