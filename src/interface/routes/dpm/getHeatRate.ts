...
import { getHeatRateController } from "../../controllers/dpm/KPI003/getHeatRateController.js"

// Function to handle the result of the HeatRate API request
const handleResult = (res: Response, heatrateJson: Either<ApplicationError, HeatRateJson>): void => {
  if (isRight(heatrateJson)) {
    logResponse(heatrateJson.right)
    res.status(HTTP_STATUS.OK).send(heatrateJson.right)
  } else {
    const error: ApplicationError = heatrateJson.left
    switch (error._tag) {
...
// Handler for the GET HeatRate API endpoint
export const getHeatRate = (router: Router): void => {
  router.get(
    "/",
    asyncWrapper(async (req, res) => {
      logger.info(`GET HeatRate API started`) // Logging a message indicating the start of the HeatRate API request
      ...
      await wrapInSnowflakeTransaction(async (snowflakeTransaction) => {
        ...
        const heatrateJson = await getHeatRateController(
          {
