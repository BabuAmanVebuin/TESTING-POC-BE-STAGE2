...
// Function to generate operationcost JSON
export const generateOperationCostResponseUseCase = async (
  plantCode: string,
  unitCode: string | null,
  timestamp: number,
  kpi003Repository: Kpi003RepositoryPort,
  t: any,
): Promise<OperationCostJson> => {
  ...
  // Fetch operationcost estimates asynchronously using Promise.all
  const [
    plannedRows,
    ...
    dailyEstimatesArray,
    operationcostCurrentFiscalYearEstimates,
    ...
    // Fetch annual operationcost estimates
    kpi003Repository.getOperationCostEstimates(
      plantCode,
      unitCode,
      "annual",
    ...
    // Fetch monthly operationcost estimates
    kpi003Repository.getOperationCostEstimates(
      plantCode,
      unitCode,
      "monthly",
    ...
    // Fetch weekly operationcost estimates
    kpi003Repository.getOperationCostEstimates(
      plantCode,
      unitCode,
      "weekly",
    ...
    // Fetch daily operationcost estimates
    kpi003Repository.getOperationCostEstimates(
      plantCode,
      unitCode,
      "daily",
    ...
    // Fetch operationcost estimates for the current fiscal year
    kpi003Repository.getOperationCostEstimates(
      plantCode,
      unitCode,
      "annual",
  ...
  // Generate the operationcost JSON object
  const returnJson: OperationCostJson = {
    PlantCode: plantCode,
    UnitCode: unitCode,
    OperationCost: generateKpi003MeasureSectionUseCase({
      ...
      unscaledCurrentYear: {
        ...
        planned: operationcostCurrentFiscalYearEstimates[0]?.planned || 0,
      },
      otherPrefix: t("VALUE.PREFIX_YEN"),
      otherSuffix: t("VALUE.SUFFIX_OKU"),
      otherConversion: (foo) => Math.ceil(foo / 1000000) / 100, // convert value into Oku & Ceil the value to two decimal place
      dailyPrefix: t("VALUE.PREFIX_YEN"),
      dailySuffix: t("VALUE.SUFFIX_MAN"),
      dailyConversion: (foo) => Math.ceil(foo / 10000), // convert value into Man & Ceil value after decimal points
    }),
  }
  return returnJson
}
