...
// Function to generate grossmargin JSON
export const generateGrossMarginResponseUseCase = async (
  plantCode: string,
  unitCode: string | null,
  timestamp: number,
  kpi003Repository: Kpi003RepositoryPort,
  t: any,
): Promise<GrossMarginJson> => {
  ...
  // Fetch grossmargin estimates asynchronously using Promise.all
  const [
    plannedRows,
    ...
    // Fetch annual grossmargin estimates
    ...
    // Fetch monthly grossmargin estimates
    kpi003Repository.getGrossMarginEstimates(
      plantCode,
      unitCode,
      "monthly",
    ...
    // Fetch weekly grossmargin estimates
    kpi003Repository.getGrossMarginEstimates(
      plantCode,
      unitCode,
      "weekly",
    ...
    // Fetch daily grossmargin estimates
    kpi003Repository.getGrossMarginEstimates(
      plantCode,
      unitCode,
      "daily",
  ...
  const returnJson: GrossMarginJson = {
    PlantCode: plantCode,
    UnitCode: unitCode,
    GrossMargin: generateKpi003MeasureSectionUseCase({
      ...
      unscaledCurrentYear: {
        ...
        planned: grossmarginCurrentFiscalYearEstimates[0]?.planned || 0,
      },
