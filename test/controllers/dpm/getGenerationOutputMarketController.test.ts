...
import { getGenerationOutputMarketController } from "../../../src/interface/controllers/dpm/KPI003/getGenerationOutputMarketController.js"
...
describe("getGenerationOutputMarketController", function () {
  this.timeout(10000)
  ...
  it("should return Left when Plant Id is empty", async () => {
    ...
    const res = await getGenerationOutputMarketController(req, snowflakeRepository, kpiResponseCacheRepository, i18n.__)
  ...
  it("should return Left if epochSeconds contain invalid numbers", async () => {
    /**
     * Test Data
     */
    const req = {
  ...
  it("should return valid response for unit", async () => {
    ...
    if (E.isRight(res)) {
      ...
      expect(Object.keys(responseData.GenerationOutputMarket)).to.include.members(KPI003MeasuresGranularityKeys)
      // Suffix response
      expect(responseData.GenerationOutputMarket.Annual.Suffix).to.equal(i18n.__("VALUE.GWH"))
      expect(responseData.GenerationOutputMarket.Monthly.Suffix).to.equal(i18n.__("VALUE.GWH"))
      expect(responseData.GenerationOutputMarket.Weekly.Suffix).to.equal(i18n.__("VALUE.GWH"))
      expect(responseData.GenerationOutputMarket.Daily.Suffix).to.equal(i18n.__("VALUE.MWH"))
      // Prefix response
      expect(responseData.GenerationOutputMarket.Annual.Prefix).to.be.null
      expect(responseData.GenerationOutputMarket.Monthly.Prefix).to.be.null
      expect(responseData.GenerationOutputMarket.Weekly.Prefix).to.be.null
      expect(responseData.GenerationOutputMarket.Daily.Prefix).to.be.null
    } else {
      throw new Error("getGenerationOutputMarketController invalid response")
    }
  })
  ...
  it("should return valid response for plant", async () => {
    ...
    if (E.isRight(res)) {
      ...
      throw Error("getGenerationOutputMarketController invalid response")
    }
  })

  it("should generate GenerationOutputMarket response when ResponseCache is empty", async () => {
    const transaction = getTransaction()
    await cleanKpi003ResponseCacheUseCase(kpiResponseCacheRepository, transaction)
    const res = await getGenerationOutputMarketController(
      {
        plantCode: "HE_",
