...
import { getSpreadMarketController } from "../../../src/interface/controllers/dpm/KPI003/getSpreadMarketController.js"
...
describe("getSpreadMarketController", function () {
  this.timeout(10000)

  before(async function () {
    return startTransaction(function (transaction) {
      return startSnowflakeTransaction(beforeFixture(transaction))
    })
  })
  ...
  it("should return Left when Plant Id is empty", async function () {
    ...
    const res = await getSpreadMarketController(req, snowflakeRepository, kpiResponseCacheRepository, i18n.__)
  ...
  it("should return valid response for unit", async function () {
    ...
    if (E.isRight(res)) {
      ...
      expect(Object.keys(responseData.SpreadMarket)).to.include.members(KPI003MeasuresGranularityKeys)
      // Suffix response
      expect(responseData.SpreadMarket.Annual.Suffix).to.equal(i18n.__("VALUE.SUFFIX_YEN_KWH"))
      expect(responseData.SpreadMarket.Monthly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_YEN_KWH"))
      expect(responseData.SpreadMarket.Weekly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_YEN_KWH"))
      expect(responseData.SpreadMarket.Daily.Suffix).to.equal(i18n.__("VALUE.SUFFIX_YEN_KWH"))
      // Prefix response
      expect(responseData.SpreadMarket.Annual.Prefix).to.be.null
      expect(responseData.SpreadMarket.Monthly.Prefix).to.be.null
      expect(responseData.SpreadMarket.Weekly.Prefix).to.be.null
      expect(responseData.SpreadMarket.Daily.Prefix).to.be.null
    } else {
      throw new Error("getSpreadMarketController invalid response")
    }
  })
  ...
  it("should generate SpreadMarket response when ResponseCache is empty", async function () {
    const transaction = getTransaction()
    await cleanKpi003ResponseCacheUseCase(kpiResponseCacheRepository, transaction)
    const res = await getSpreadMarketController(
      {
        plantCode: "HE_",
