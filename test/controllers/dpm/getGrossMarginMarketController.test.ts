...
import { getGrossMarginMarketController } from "../../../src/interface/controllers/dpm/KPI003/getGrossMarginMarketController.js"
...
describe("getGrossMarginMarketController", function () {
  this.timeout(10000)
  ...
  it("should return Left when Plant Id is empty", async function () {
    ...
    const res = await getGrossMarginMarketController(req, snowflakeRepository, kpiResponseCacheRepository, i18n.__)
  ...
  it("should return Left if plantCode is invalid and unitCode is valid", async function () {
    /**
     * Test Data
     */
    const req = {
  ...
  it("should return Left if plantCode is valid but unitCode is not valid", async function () {
    /**
     * Test Data
     */
    const req = {
  ...
  it("should return Left if epochSeconds are invalid numbers", async function () {
    /**
     * Test Data
     */
    const req = {
  ...
  it("should return valid response for unit", async function () {
    ...
    if (E.isRight(res)) {
      ...
      expect(Object.keys(responseData.GrossMarginMarket)).to.include.members(KPI003MeasuresGranularityKeys)
      // Suffix response
      expect(responseData.GrossMarginMarket.Annual.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.GrossMarginMarket.Monthly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.GrossMarginMarket.Weekly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.GrossMarginMarket.Daily.Suffix).to.equal(i18n.__("VALUE.SUFFIX_MAN"))
      // Prefix response
      expect(responseData.GrossMarginMarket.Annual.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.GrossMarginMarket.Monthly.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.GrossMarginMarket.Weekly.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.GrossMarginMarket.Daily.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
    } else {
      throw new Error("getGrossMarginMarketController invalid response")
    }
  })
  ...
  it("should generate GrossMarginMarket response when ResponseCache is empty", async function () {
    const transaction = getTransaction()
    await cleanKpi003ResponseCacheUseCase(kpiResponseCacheRepository, transaction)
    const res = await getGrossMarginMarketController(
      {
        plantCode: "HE_",
