...
import { getGrossMarginPPAController } from "../../../src/interface/controllers/dpm/KPI003/getGrossMarginPPAController.js"
...
describe("getGrossMarginPPAController", function () {
  this.timeout(10000)
  ...
  it("should return Left when Plant Id is empty", async () => {
    ...
    const res = await getGrossMarginPPAController(req, snowflakeRepository, kpiResponseCacheRepository, i18n.__)
  ...
  it("should return valid response for unit", async () => {
    ...
    if (E.isRight(res)) {
      ...
      expect(Object.keys(responseData.GrossMarginPPA)).to.include.members(KPI003MeasuresGranularityKeys)

      // Suffix response
      expect(responseData.GrossMarginPPA.Annual.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.GrossMarginPPA.Monthly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.GrossMarginPPA.Weekly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.GrossMarginPPA.Daily.Suffix).to.equal(i18n.__("VALUE.SUFFIX_MAN"))

      // Prefix response
      expect(responseData.GrossMarginPPA.Annual.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.GrossMarginPPA.Monthly.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.GrossMarginPPA.Weekly.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.GrossMarginPPA.Daily.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
    } else {
      throw new Error("getGrossMarginPPAController invalid response")
    }
  })
  ...
  it("should return valid response for plant", async () => {
    ...
    if (E.isRight(res)) {
      ...
      throw Error("getGrossMarginPPAController invalid response")
    }
  })

  it("should generate GrossMarginPPA response when ResponseCache is empty", async () => {
    const transaction = getTransaction()
    await cleanKpi003ResponseCacheUseCase(kpiResponseCacheRepository, transaction)

    const res = await getGrossMarginPPAController(
      {
        plantCode: "HE_",
