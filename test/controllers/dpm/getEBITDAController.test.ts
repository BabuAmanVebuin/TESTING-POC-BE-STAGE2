...
import { getEBITDAController } from "../../../src/interface/controllers/dpm/KPI003/getEBITDAController.js"
...
describe("getEBITDAController", function () {
  this.timeout(10000)
  ...
  it("should return Left when Plant Id is empty", async () => {
    ...
    const res = await getEBITDAController(req, snowflakeRepository, kpiResponseCacheRepository, i18n.__)
  ...
  it("should return valid response for unit", async () => {
    ...
    if (E.isRight(res)) {
      ...
      expect(Object.keys(responseData.EBITDA)).to.include.members(KPI003MeasuresGranularityKeys)
      // Suffix response
      expect(responseData.EBITDA.Annual.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.EBITDA.Monthly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.EBITDA.Weekly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.EBITDA.Daily.Suffix).to.equal(i18n.__("VALUE.SUFFIX_MAN"))
      // Prefix response
      expect(responseData.EBITDA.Annual.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.EBITDA.Monthly.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.EBITDA.Weekly.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.EBITDA.Daily.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
    } else {
  ...
  it("should generate EBITDA response when ResponseCache is empty", async () => {
    const transaction = getTransaction()
    await cleanKpi003ResponseCacheUseCase(kpiResponseCacheRepository, transaction)
    const res = await getEBITDAController(
      {
        plantCode: "HE_",
