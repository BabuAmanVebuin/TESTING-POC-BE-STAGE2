...
import { getBasicProfitController } from "../../../src/interface/controllers/dpm/KPI003/getBasicProfitController.js"
...
describe("getBasicProfitController", function () {
  this.timeout(10000)
  ...
  it("should return Left when Plant Id is empty", async () => {
    ...
    const res = await getBasicProfitController(req, snowflakeRepository, kpiResponseCacheRepository, i18n.__)
  ...
  it("should return valid response for unit", async () => {
    ...
    if (E.isRight(res)) {
      ...
      expect(Object.keys(responseData.BasicProfit)).to.include.members(KPI003MeasuresGranularityKeys)
      // Suffix response
      expect(responseData.BasicProfit.Annual.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.BasicProfit.Monthly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.BasicProfit.Weekly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.BasicProfit.Daily.Suffix).to.equal(i18n.__("VALUE.SUFFIX_MAN"))
      // Prefix response
      expect(responseData.BasicProfit.Annual.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.BasicProfit.Monthly.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.BasicProfit.Weekly.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.BasicProfit.Daily.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
    } else {
      throw new Error("getBasicProfitController invalid response")
    }
  })
  ...
  it("should generate BasicProfit response when ResponseCache is empty", async () => {
    const transaction = getTransaction()
    await cleanKpi003ResponseCacheUseCase(kpiResponseCacheRepository, transaction)
    const res = await getBasicProfitController(
      {
        plantCode: "HE_",
