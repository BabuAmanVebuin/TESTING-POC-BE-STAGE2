...
import { getAvailabilityController } from "../../../src/interface/controllers/dpm/KPI003/getAvailabilityController.js"
...
describe("getAvailabilityController", function () {
  this.timeout(10000)
  ...
  it("should return Left when Plant Id is empty", async function () {
    ...
    const res = await getAvailabilityController(req, snowflakeRepository, kpiResponseCacheRepository, i18n.__)
  ...
  it("should return valid response for unit", async function () {
    ...
    if (E.isRight(res)) {
      ...
      expect(Object.keys(responseData.Availability)).to.include.members(KPI003MeasuresGranularityKeys)
      // Suffix response
      expect(responseData.Availability.Annual.Suffix).to.equal(i18n.__("VALUE.SUFFIX_PERCENTAGE"))
      expect(responseData.Availability.Monthly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_PERCENTAGE"))
      expect(responseData.Availability.Weekly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_PERCENTAGE"))
      expect(responseData.Availability.Daily.Suffix).to.equal(i18n.__("VALUE.SUFFIX_PERCENTAGE"))
      // Prefix response
      expect(responseData.Availability.Annual.Prefix).to.be.null
      expect(responseData.Availability.Monthly.Prefix).to.be.null
      expect(responseData.Availability.Weekly.Prefix).to.be.null
      expect(responseData.Availability.Daily.Prefix).to.be.null
    } else {
      throw new Error("getAvailabilityController invalid response")
    }
  })
  ...
  it("should generate Availability response when ResponseCache is empty", async function () {
    const transaction = getTransaction()
    await cleanKpi003ResponseCacheUseCase(kpiResponseCacheRepository, transaction)
    const res = await getAvailabilityController(
      {
        plantCode: "HE_",
