...
import { getHeatRateController } from "../../../src/interface/controllers/dpm/KPI003/getHeatRateController.js"
...
describe("getHeatRateController", function () {
  this.timeout(10000)
  ...
  it("should return Left when Plant Id is empty", async () => {
    ...
    const res = await getHeatRateController(req, snowflakeRepository, kpiResponseCacheRepository, i18n.__)
  ...
  it("should return valid response for unit", async () => {
    ...
    if (E.isRight(res)) {
      ...
      expect(Object.keys(responseData.HeatRate)).to.include.members(KPI003MeasuresGranularityKeys)
      // Suffix response
      expect(responseData.HeatRate.Annual.Suffix).to.equal(i18n.__("VALUE.SUFFIX_KJ_KWh"))
      expect(responseData.HeatRate.Monthly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_KJ_KWh"))
      expect(responseData.HeatRate.Weekly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_KJ_KWh"))
      expect(responseData.HeatRate.Daily.Suffix).to.equal(i18n.__("VALUE.SUFFIX_KJ_KWh"))
      // Prefix response
      expect(responseData.HeatRate.Annual.Prefix).to.be.null
      expect(responseData.HeatRate.Monthly.Prefix).to.be.null
      expect(responseData.HeatRate.Weekly.Prefix).to.be.null
      expect(responseData.HeatRate.Daily.Prefix).to.be.null
    } else {
      throw new Error("getHeatRateController invalid response")
    }
  })
  ...
  it("should generate HeatRate response when ResponseCache is empty", async () => {
    const transaction = getTransaction()
    await cleanKpi003ResponseCacheUseCase(kpiResponseCacheRepository, transaction)
    const res = await getHeatRateController(
      {
        plantCode: "HE_",
