...
import { getOperationCostController } from "../../../src/interface/controllers/dpm/KPI003/getOperationCostController.js"
...
describe("getOperationCostController", function () {
  this.timeout(10000)
  ...
  it("should return Left when Plant Id is empty", async () => {
    ...
    const res = await getOperationCostController(req, snowflakeRepository, kpiResponseCacheRepository, i18n.__)
  ...
  it("should return valid response for unit", async () => {
    ...
    if (E.isRight(res)) {
      ...
      expect(Object.keys(responseData.OperationCost)).to.include.members(KPI003MeasuresGranularityKeys)
      // Suffix response
      expect(responseData.OperationCost.Annual.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.OperationCost.Monthly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.OperationCost.Weekly.Suffix).to.equal(i18n.__("VALUE.SUFFIX_OKU"))
      expect(responseData.OperationCost.Daily.Suffix).to.equal(i18n.__("VALUE.SUFFIX_MAN"))
      // Prefix response
      expect(responseData.OperationCost.Annual.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.OperationCost.Monthly.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.OperationCost.Weekly.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
      expect(responseData.OperationCost.Daily.Prefix).to.equal(i18n.__("VALUE.PREFIX_YEN"))
    } else {
      throw new Error("getOperationCostController invalid response")
    }
  })
  ...
  it("should generate OperationCost response when ResponseCache is empty", async () => {
    const transaction = getTransaction()
    await cleanKpi003ResponseCacheUseCase(kpiResponseCacheRepository, transaction)
    const res = await getOperationCostController(
      {
        plantCode: "HE_",
