...
import { getGenerationOutputController } from "../../../src/interface/controllers/dpm/KPI003/getGenerationOutputController.js"
...
describe("getGenerationOutputController", function () {
  this.timeout(10000)
  ...
  it("should return Left when Plant Id is empty", async () => {
    ...
    const res = await getGenerationOutputController(req, snowflakeRepository, kpiResponseCacheRepository, i18n.__)
  ...
  it("should return valid response for unit", async () => {
    ...
    if (E.isRight(res)) {
      ...
      expect(Object.keys(responseData.GenerationOutput)).to.have.members(KPI003MeasuresGranularityKeys)
      // Suffix response
      expect(responseData.GenerationOutput.Annual.Suffix).to.equal(i18n.__("VALUE.GWH"))
      expect(responseData.GenerationOutput.Monthly.Suffix).to.equal(i18n.__("VALUE.GWH"))
      expect(responseData.GenerationOutput.Weekly.Suffix).to.equal(i18n.__("VALUE.GWH"))
      expect(responseData.GenerationOutput.Daily.Suffix).to.equal(i18n.__("VALUE.MWH"))
      // Prefix response
      expect(responseData.GenerationOutput.Annual.Prefix).to.be.null
      expect(responseData.GenerationOutput.Monthly.Prefix).to.be.null
      expect(responseData.GenerationOutput.Weekly.Prefix).to.be.null
      expect(responseData.GenerationOutput.Daily.Prefix).to.be.null
    } else {
      throw new Error("getGenerationOutputController invalid response")
    }
  })
  ...
  it("should generate GenerationOutput response when ResponseCache is empty", async () => {
    const transaction = getTransaction()
    await cleanKpi003ResponseCacheUseCase(kpiResponseCacheRepository, transaction)
    const res = await getGenerationOutputController(
      {
        plantCode: "HE_",
