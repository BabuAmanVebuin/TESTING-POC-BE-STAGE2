...
const ENDPOINT_PATH = "/basic-charge/forecast"

// Insert fixtures before each test
const beforeHookFixtures = async (transaction: Transaction) => {
  await insertFixture("beforeUpsertBasicChargeForecast.sql", transaction)
}

const basicChargeForecastInputToBeInserted = [
  {
    "plant-id": "KS_",
    "unit-id": "KS_A100",
    "fiscal-year": 2098,
...
const basicChargeForecastInputToBeUpdated = [
  {
    "plant-id": "HK_",
    "unit-id": "HK_A100",
    "fiscal-year": 2099,
...
describe("Basic charge forecast tests", function () {
  ...
  describe("UPSERT basic charge forecast", function () {
    it("Valid request: should insert new data", async () => {
      const res = await request(app)
        .put(`${ENDPOINT_PATH}`)
        .set("accept-language", "en")
        .send(basicChargeForecastInputToBeInserted)
    ...
    it("Valid request: should update new data", async () => {
      const res = await request(app)
        .put(`${ENDPOINT_PATH}`)
        .set("accept-language", "en")
        .send(basicChargeForecastInputToBeUpdated)
      expect(res.status).to.eql(200)
      expect(res.body).to.be.a("string")
      expect(res.body).to.equal("OK")
    })
    ...
    it("Bad request: missing required fields: plant-id", async () => {
      const badBasicChargeForecastInput = [
        {
          "unit-id": "HE_A100",
          "fiscal-year": 2021,
          "operation-input": 344.34,
      ...
      const res = await request(app)
        .put(`${ENDPOINT_PATH}`)
        .set("accept-language", "en")
        .send(badBasicChargeForecastInput)
  ...
  describe("GET basic charge forecast", function () {
    describe("Should Ensure that UPSERT API has worked correctly", function () {
      it("Valid request: should ensure that upsert has inserted new data", async () => {
        ...
        expect(res.body[0].sum).eq(
          basicChargeForecastInputToBeInserted[0]["operation-input"] +
            basicChargeForecastInputToBeInserted[0]["maintenance-input"],
        )
      })
      it("Valid request: should ensure that upsert has updated old data", async () => {
        const res = await request(app).get(`${ENDPOINT_PATH}`).set("accept-language", "en").query({
          "plant-id": "HK_",
          "unit-id": "HK_A100",
          "start-fiscal-year": 2099,
          "end-fiscal-year": 2099,
        })
        expect(res.status).to.eql(200)
        expect(res.body).to.be.an("array")
    ...
    it("Valid request: should be able to filter by fiscal-year correctly", async () => {
      ...
      expect(res2.body.length).to.eq(17)
    })
